...
public class PostControllerTest {
    ...
    @Test
    @WithMockUser
    void 포스트수정시_본인이작성한포스트가아니라면_에러발생() throws Exception {
        String title = "title";
        String body = "body";

        // TODO
        // 본인이 작성한 포스트가 아니라는 내용 필요 -> mocking
    ...
    @Test
    @WithMockUser
    void 포스트수정시_수정하려는포스트가없는경우_에러발생() throws Exception {
        String title = "title";
        String body = "body";

        // TODO
        // 수정하려는 포스트가 없다는 내용 필요 -> mocking
    ...
    @Test
    @WithAnonymousUser
    void 포스트삭제시_로그인하지않은경우_에러발생() throws Exception {
        mvc.perform(delete("/api/v1/posts/1")
                        .contentType(MediaType.APPLICATION_JSON)
                ).andDo(print())
                .andExpect(status().isUnauthorized());
    ...
    @Test
    @WithMockUser
    void 포스트삭제시_작성자와삭제요청자가다르면_에러발생() throws Exception {
        // mocking
    ...
    @Test
    @WithMockUser
    void 포스트삭제시_삭제하려는포스트가없는경우_에러발생() throws Exception {
        // mocking
    ...
    @Test
    @WithAnonymousUser
    void 피드목록요청시_로그인하지않은경우_에러발생() throws Exception {
        // mocking
        when(postService.list(any())).thenReturn(Page.empty());
    ...
    @Test
    @WithAnonymousUser
    void 내피드목록요청시_로그인하지않은경우_에러발생() throws Exception {
        // mocking
        when(postService.my(any(), any())).thenReturn(Page.empty());
    ...
    @Test
    @WithAnonymousUser
    void 좋아요버튼클릭시_로그인하지않은경우_에러발생() throws Exception {
        mvc.perform(post("/api/v1/posts/1/likes")
                        .contentType(MediaType.APPLICATION_JSON)
                ).andDo(print())
                .andExpect(status().isUnauthorized());
    ...
    @Test
    @WithAnonymousUser
    void 좋아요버튼클릭시_포스트가없는경우_에러발생() throws Exception {
    ...
    @Test
    @WithAnonymousUser
    void 댓글작성시_로그인하지않은경우_에러발생() throws Exception {
        mvc.perform(post("/api/v1/posts/1/comments")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsBytes(new PostCommentRequest("comment")))
                ).andDo(print())
    ...
    @Test
    @WithAnonymousUser
    void 댓글작성시_포스트가없는경우_에러발생() throws Exception {
